...
function patch_serviceaccount_noi_operator_catalog () {
...
while [[ ${RESOURCE_FOUND} == "false" && $LOOP_COUNT -lt $MAX_LOOP_COUNT ]]; do
    ...
    RESOURCE_COUNT=$(oc get serviceaccount noi-operator-catalog -n openshift-marketplace | wc -l)
...
then
    echo "Resource found (serviceaccount noi-operator-catalog)"
    echo "Patch serviceaccount with imagePullSecrets"
    oc patch -n openshift-marketplace serviceaccount noi-operator-catalog -p '{"imagePullSecrets": [{"name": "ibm-entitlement-key"}]}'
    ...
    echo "Resource Not found (serviceaccount noi-operator-catalog)"
fi
}


## Call the Main method
patch_serviceaccount_noi_operator_catalog