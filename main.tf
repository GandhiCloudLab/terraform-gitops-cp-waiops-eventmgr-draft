locals {
  ...
  values_content_subscription = {
    ...
    subscription_startingCSV = var.subscription_startingCSV
  }
...
module service_account {
  ...
  pull_secrets = [local.image_pullsecret_name]
}
...
resource null_resource run_patch_service_account l {
  depends_on = [null_resource.setup_gitops_subscription]

  provisioner "local-exec" {
    command = "${path.module}/scripts/run-patch-service-account.sh"
...
resource "null_resource" "create_yaml_instance" {
    depends_on = [null_resource.run_patch_service_account]
